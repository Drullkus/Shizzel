...
public class GUIChiselingTerminalCopy extends AbstractGUIBase implements ISortSource
{
    ...
    private static final int GUI_HEIGHT = GUI_UPPER_HEIGHT + GUI_MIDDLE_HEIGHT + GUI_LOWER_HEIGHT;
    ...
    public GUIChiselingTerminalCopy(PartChiselingTerminal part, EntityPlayer entPlayer)
    {
    ...
    protected void drawGuiContainerBackgroundLayer(float alpha, int cursorX, int cursorY)
    {
        ...
        for (int i = 0; i < (this.widgetRows - ME_ROWS); ++i)
        {
    ...
    /*private void setupTerminalStyle()
    {
        ...
        this.ySize = GUI_HEIGHT + ( extraRows * GUI_MIDDLE_HEIGHT );
        this.guiTop = ( this.height - this.ySize ) / 2;

        // Update the number of widgetRows
        this.widgetRows = ME_DEFAULT_ROWS + extraRows;

        // Update number of widgets
        this.widgetCount = this.widgetRows * ME_COLUMNS;
